...
export async function startDualPriceMonitor(
  ...
  if (!kalshiTicker) {
    ...
    if (markets.length === 0) {
      const errorMsg = "No open Bitcoin up/down markets found. This may be normal if markets are closed or between 15-minute slots. The bot will retry on the next poll cycle.";
      console.warn(`[Monitor] ${errorMsg}`);
      if (options.onError) {
        options.onError(new Error(errorMsg));
      }
      // Return a no-op cleanup function instead of throwing
      // This allows the monitor to be restarted later when markets are available
      return () => {};
    }
    kalshiTicker = markets[0].ticker;
    console.log(`[Monitor] Using Kalshi market: ${kalshiTicker}`);
  }
